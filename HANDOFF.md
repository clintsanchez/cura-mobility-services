...
## No custom code needed (2026-09-26, WP Engine)
WPCodeBox snippet 1 ("Custom PHP") is **disabled** and nothing depends on it (snippet 2 was already off; both can be deleted). What replaced each job:
- **Comments:** Settings → Discussion closed for new posts; all existing posts closed; no comment elements in templates.
- **Resources title:** Resources (1547) is now a normal Breakdance page (Reviews layout + post-card loop, custom query, 9 per page, number pagination); `page_for_posts` = 0. Template 454 now only serves category archives. Pagination untested (only 3 posts).
- **"Category:" prefix:** template 454 uses Archive Title with `disable_prefix='true'`.
- **Email/phone:** only in Settings → Business info, read by Breakdance dynamic data (header, footer, buttons, sidebars, Contact page). Text fields no longer carry the number: FAQ answers, confirmations and service-area text say "Call us" (a call button is beside each); policies say "Phone and email: see our Contact page" (sidebar shows them); SEO descriptions dropped the number. Script: `website/build/cura-no-snippet-text.php`.
- **Blog posts:** number hard-coded again in the 3 posts (`blog/scripts/render_post.py` writes it plain). When the number changes: update `blog/posts/*.md`, re-render, run `website/build/cura-blog-update.php`, and remake the featured images.
- **Still static:** WS Form notification recipients.
